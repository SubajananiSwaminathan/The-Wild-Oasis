...
import { formatDistanceFromNow, formatCurrency } from "../../utilities/helpers";
...
function BookingDataBox({ booking }) {
  const {
    ...
    observations,
    is_paid,
    guests: { full_name: guestName, email, country, country_flag, nic_no },
    ...
    <StyledBookingDataBox>
      <Header>
        <div>
          <HiOutlineHomeModern />
          <p>
            {no_of_nights} nights in Cabin <span>{cabinName}</span>
          </p>
        </div>
      ...
      <Section>
        <Guest>
          ...
          <p>
            {guestName} {no_of_guests > 1 ? `+ ${no_of_guests - 1} guests` : ""}
          </p>
          <span>&bull;</span>
          <p>{email}</p>
          <span>&bull;</span>
          <p>National ID {nic_no}</p>
        </Guest>
